...
class ProductSpider(Spider):
    ...
    def parse(self, response: HtmlResponse, **kwargs):
        print(f"Starting Scraper on date: {datetime.today()}")
    ...
    def parse_product(self, response, url, product_name):
        ...
        try:
            ...
            product["url"] = url
            product["code"] = (
                response.css("p#barcode_paragraph").css("span::text").get()
            )
        ...
        # pylint: disable=broad-exception-caught
            ...
            print(
                f"There was an error with scraping the product from the following url: {url}"
            )
